...
function ratingFromStarsText(starsText) {
  const s = String(starsText || "");
  const filled = (s.match(/★/g) || []).length;
  return filled || null;
}

function isoFromTimeText(timeText) {
  const m = String(timeText || "").trim().match(/^(\d{4}-\d{2}-\d{2})\s+(\d{2}:\d{2})$/);
  if (!m) return null;
...
function extractDataAttributesFromTrOpenTag(trOpenTag) {
  const attrs = {};
  const tag = String(trOpenTag || "");
...
function extractReviewsJsonArrayFromHtml(html) {
  const tbody = extractFirst(
    html,
    /<tbody[^>]*\bid=["']boardBody["'][^>]*>([\s\S]*?)<\/tbody>/i
  );
  if (!tbody) return [];

  const rowRe = /<tr\b[^>]*class=["'][^"']*\bboard__row\b[^"']*["'][^>]*>[\s\S]*?<\/tr>/gi;
  const out = [];
  let rowMatch;
  while ((rowMatch = rowRe.exec(tbody))) {
    const rowHtml = rowMatch[0];
    const trOpen = extractFirst(rowHtml, /^(<tr\b[^>]*>)/i);
    const dataAttributes = extractDataAttributesFromTrOpenTag(trOpen);

    const id = dataAttributes["data-id"] || "";
    const contentAttr = dataAttributes["data-content"] || "";
    const authorRaw = dataAttributes["data-author-raw"] || "";

    const titleRaw = extractFirst(
      rowHtml,
      /<button[^>]*\bclass=["'][^"']*\blinkTitle\b[^"']*["'][^>]*>([\s\S]*?)<\/button>/i
    );
    const previewRaw = extractFirst(
      rowHtml,
      /<div[^>]*\bclass=["'][^"']*\bpreview\b[^"']*["'][^>]*>([\s\S]*?)<\/div>/i
    );
    const authorMaskedRaw = extractFirst(
      rowHtml,
      /<td[^>]*\bclass=["'][^"']*\bcell-author\b[^"']*["'][^>]*>([\s\S]*?)<\/td>/i
    );
    const timeTextRaw = extractFirst(
      rowHtml,
      /<td[^>]*\bclass=["'][^"']*\bcell-time\b[^"']*["'][^>]*>([\s\S]*?)<\/td>/i
    );
    const starsRaw = extractFirst(
      rowHtml,
      /<span[^>]*\bclass=["'][^"']*\bstars\b[^"']*["'][^>]*>([\s\S]*?)<\/span>/i
    );

    const title = stripTags(decodeHtmlEntities(titleRaw)).trim();
    const preview = stripTags(decodeHtmlEntities(previewRaw)).trim();
    const authorMasked = stripTags(decodeHtmlEntities(authorMaskedRaw)).trim();
    const timeText = stripTags(decodeHtmlEntities(timeTextRaw)).trim();
    const starsText = stripTags(decodeHtmlEntities(starsRaw)).trim();

    const fullText = stripTags(decodeHtmlEntities(contentAttr || preview)).trim();
    const rating = ratingFromStarsText(starsText);
    const date = isoFromTimeText(timeText);
    ...
    out.push({
      ...
      dataAttributes: {
        ...dataAttributes,
        ...(authorRaw ? { "data-author-raw": authorRaw } : {}),
      },
    });
  }
...
function writeReviewsJsonFileFromHtml(html) {
  const arr = extractReviewsJsonArrayFromHtml(html);
  fs.mkdirSync(path.dirname(REVIEWS_JSON), { recursive: true });
  fs.writeFileSync(REVIEWS_JSON, JSON.stringify(arr, null, 2) + "\n", "utf8");
}
...
function run() {
  ...
  if (html.includes(todayIdPrefix)) {
    writeReviewsJsonFileFromHtml(html);
  ...
  writeReviewsJsonFileFromHtml(next);
